...
bellow solutins runtime: 4ms, memory 2.97mb
*/

func main() {
	fmt.Println("hello romans")
	fmt.Println("expected: 3; returned: ", romanToInt("III"))
	fmt.Println("expected: 58; returned: ", romanToInt("LVIII"))
	fmt.Println("expected: 1994; returned: ", romanToInt("MCMXCIV"))
}

func romanToInt(s string) int {
	var numPairs = map[byte]int{
		'I': 1,
		'V': 5,
		'X': 10,
		'L': 50,
	...
	for i := 0; i < len(s); i++ {
		if i < len(s) - 1 && numPairs[s[i]] < numPairs[s[i+1]] {
			total -= numPairs[s[i]]
		} else {
			total += numPairs[s[i]]
		}
